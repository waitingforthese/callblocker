...
class AlarmReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val gocharTaraAaradhana = id in 401..409

        if (isGuidanceNotification) {
            showNakshatraGuidanceNotification(context, id, eventAt)
        } else if (!gocharTaraAaradhana) {
            showNotification(context, title, message, id, eventAt)
        }

        if (gocharTaraAaradhana) {
            val pendingResult = goAsync()
            val appContext = context.applicationContext
            Thread {
                try {
                    val profile = BirthProfileStore.load(appContext)
                    val graha = Graha.entries[id - 401]
                    val gp = GocharTaraAaradhanaPrefs(appContext)
                    val current = profile?.let { GocharTaraAaradhanaCalculator.current(it).firstOrNull { row -> row.graha == graha } }
                    val valid = profile != null && current != null &&
                        gp.isEnabled(graha) && gp.selectedTaraNames(graha).contains(current.tara)
                    if (valid) {
                        val info = AaradhanaMaster.forGraha(graha.marathi)
                        val currentInfo = current!!
                        val text = "${graha.marathi} — ${currentInfo.nakshatra} • तारा: ${currentInfo.tara}"
                        showGocharTaraNotification(context, graha, text, eventAt)
                        AaradhanaVoiceSession.speakRepeated(appContext, id, info.mantra, gpJapaCount(appContext), pendingResult) {
                            runCatching { if (wakeLock.isHeld) wakeLock.release() }
                        }
                    } else {
                    ...
                    android.util.Log.e("LifeAlarm", "Gochar Tara Aaradhana failed", t)
                    pendingResult.finish()
                    runCatching { if (wakeLock.isHeld) wakeLock.release() }
                }
            }.start()
        ...
        if (id in 1..3 || id in 11..13 || id in 21..27 || id == 121 || id == 122 || id in 131..133 || id in 301..308 || id in 401..409) {
            Thread {
                try {
    ...
    private fun gpJapaCount(context: Context): Int =
        com.mahaesuvidha.chandrapanchangalarm.settings.AaradhanaPrefs(context).specialJapaCount

    private fun showGocharTaraNotification(context: Context, graha: Graha, text: String, eventAt: Long) {
        val channelId = "life_alarm_nakshatra_guidance_voice_v3"
        val manager = context.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(channelId, "गोचर तारा आराधना", NotificationManager.IMPORTANCE_HIGH)
            channel.description = "विपत, प्रत्यारी आणि वध तारा प्रवेश आराधना"
        ...
        val full = "$text\n\nआराधना: ${AaradhanaMaster.forGraha(graha.marathi).mantra}\nबदलाची वेळ: ${VoiceAnnouncement.formatEventTiming(eventAt)}"
        val notification = NotificationCompat.Builder(context, channelId)
            .setSmallIcon(android.R.drawable.ic_dialog_info)
            .setContentTitle("🕉️ ${graha.marathi} गोचर तारा आराधना")
            .setContentText(text)
            ...
            .setShowWhen(true)
            .setWhen(eventAt.takeIf { it > 0L } ?: System.currentTimeMillis())
            .setAutoCancel(true)
            .build()
        manager.notify(8000 + graha.ordinal, notification)
    }
